...
CHART_DIR = Path("/home/claude/stock_predictor/charts")
...
def plot_price_indicators(df: pd.DataFrame, ticker: str,
                           backtest_df: pd.DataFrame = None) -> str:
    """
    4-panel chart:
    ...
    fig, axes = plt.subplots(4, 1, figsize=(14, 12),
                              ...
                              facecolor=BG)
    fig.suptitle(f"{ticker} — Price, Indicators & Signals",
    ...
    ax_price.plot(dates, df["Close"], color=C_LINE, linewidth=1.2, label="Close", zorder=3)
    if "BB_Upper" in df.columns:
        ax_price.plot(dates, df["BB_Upper"],  color=C_BAND, linewidth=0.8, alpha=0.7, label="BB Upper")
        ax_price.plot(dates, df["BB_Lower"],  color=C_BAND, linewidth=0.8, alpha=0.7, label="BB Lower")
        ax_price.fill_between(dates, df["BB_Upper"], df["BB_Lower"],
                               alpha=0.08, color=C_BAND)
    if "SMA_20" in df.columns:
        ax_price.plot(dates, df["SMA_20"], color="#f59e0b", linewidth=0.9,
                       linestyle="--", label="SMA 20", alpha=0.85)
    if "EMA_12" in df.columns:
        ax_price.plot(dates, df["EMA_12"], color="#a78bfa", linewidth=0.9,
                       linestyle=":", label="EMA 12", alpha=0.85)
    ...
    if backtest_df is not None:
        ...
        if buy_mask.any():
            ax_price.scatter(bt.index[buy_mask],
                              df.loc[bt.index[buy_mask], "Close"],
                              marker="^", color=C_UP, s=50, zorder=5, label="BUY")
        if sell_mask.any():
            ax_price.scatter(bt.index[sell_mask],
                              df.loc[bt.index[sell_mask], "Close"],
                              marker="v", color=C_DOWN, s=50, zorder=5, label="SELL")

    ax_price.set_ylabel("Price (USD)", color="#9ca3af", fontsize=9)
    ax_price.legend(loc="upper left", fontsize=7.5, facecolor="#1f2937",
    ...
    if "Vol_MA20" in df.columns:
        ax_vol.plot(dates, df["Vol_MA20"] / 1e6, color="#fbbf24",
                    linewidth=0.9, label="Vol MA20")
    ...
    ax_rsi.fill_between(dates, df["RSI"], 70,
                         where=df["RSI"] >= 70, alpha=0.2, color=C_DOWN)
    ax_rsi.fill_between(dates, df["RSI"], 30,
                         where=df["RSI"] <= 30, alpha=0.2, color=C_UP)
    ...
    if "MACD" in df.columns:
        ...
        ax_macd.bar(dates, df["MACD_Hist"], color=hist_colors, alpha=0.7, width=1, label="Histogram")
        ax_macd.plot(dates, df["MACD"],        color="#38bdf8", linewidth=0.9, label="MACD")
        ax_macd.plot(dates, df["MACD_Signal"], color="#fb923c", linewidth=0.9, label="Signal")
        ax_macd.axhline(0, color="#6b7280", linewidth=0.5)
    ax_macd.set_ylabel("MACD", color="#9ca3af", fontsize=9)
    ax_macd.legend(loc="upper left", fontsize=7, facecolor="#1f2937",
                   labelcolor="white", framealpha=0.8)
...
def plot_backtest(backtest_df: pd.DataFrame, ticker: str,
                  initial_capital: float = 10_000.0) -> str:
    """
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 7),
                                    ...
                                    facecolor=BG)
    fig.suptitle(f"{ticker} — Backtest: Strategy vs. Buy & Hold",
    ...
    ax1.plot(backtest_df.index, backtest_df["Equity"], color=C_UP,
             linewidth=1.4, label="ML Strategy")
    ax1.plot(backtest_df.index, bh, color=C_BAND,
             linewidth=1.2, linestyle="--", alpha=0.9, label="Buy & Hold")
    ax1.axhline(initial_capital, color="#6b7280", linewidth=0.6, linestyle=":")
    ax1.fill_between(backtest_df.index, backtest_df["Equity"], bh,
                     where=backtest_df["Equity"] >= bh,
                     alpha=0.12, color=C_UP, label="Outperform")
    ax1.fill_between(backtest_df.index, backtest_df["Equity"], bh,
                     where=backtest_df["Equity"] < bh,
                     alpha=0.12, color=C_DOWN, label="Underperform")
...
def plot_feature_importance(model, ticker: str, top_n: int = 15) -> str:
    ...
    ax.set_title(f"{ticker} — Top {top_n} Features",
                 color="white", fontsize=11, fontweight="bold")
    ax.tick_params(axis="y", labelsize=8)

    for bar, val in zip(bars, imp.values):
        ax.text(bar.get_width() + 0.001, bar.get_y() + bar.get_height() / 2,
                f"{val:.3f}", va="center", color="#d1d5db", fontsize=7)
